...
At Mango we validate and test packages for our ValidR customers. This
requires using `testthat` to create unit tests for different requirements
of functions. These tests must run on a customers machine; hence, all tests
...
do this is by using the method of mocking.


## Mocking

Mocking allows you to replace parts of your system under test with mock
objects and make assertions about how they have been used. It allows us to
temporarily change a function, this is useful in our case, as we can remove
any attempts to connect to the internet. In `testthat` we do this using the
function `with_mock`.
...
This is a simple example where we have used `with_mock` to temporarily
amend to the function `print`. It takes only two arguments, the first
`print = function(...) paste0("Mak...` is amending the function `print` to
do as we want, the second `{print("hello")}` is using this new function
`print`. Calling this function outside of `with_mock`, will result in it
working as it should.

## Using `with_mock` to avoid internet

### Example - `GET`
To explain this, consider the function `GET` from the `httr` package, which
will get a URL;
...
`GET` uses different functions, which in turn use other functions to
connect to the internet and get this URL. Thus, the connection to the
internet doesn't occur directly from GET, but from a function
`curl_fetch_memory` from the `curl` package, which is called much deeper
into the code. Hence, to call the function `GET` without connecting to the
internet we must mock the function `curl_fetch_memory`.
...
* Record the response of this function (allowing an internet connection)
  and write it locally
* Pass the URL to the mocked function and respond with the recorded output
* Check `GET` (with the mocked `curl_fetch_memory`) works as it should and
  test it


Firstly, we need to find the output to `curl_fetch_memory` which is passed
to the function `GET`. To do this we use `trace`;
...
Then, `ret` is the response of `curl_fetch_memory`, when it is called by
`GET("http://httpbin.org/get")`, so using `dput` we can copy over the
output and save it as a variable `recorded_response`.
...
Now that we have the response stored locally we can begin to mock the
function `curl_fetch_memory`, by adding the URL that is passed to the
function `curl_fetch_memory` to a variable `myurl` and using
`recorded_response` as the output of the mock function.
...
The function `curl_fetch_memory` has been edited to return the exact result
that it would, if the function `GET("http://httpbin.org/get")` had been
called, but without connecting to the internet. Now, we can add in
...
will need to mock this function. This function only runs if `interactive ==
TRUE`, which it will do when writing the tests. However, the validation
packages are then run through Jenkins where `interactive == FALSE`, so we
also need to mock this function to always be `TRUE`. This would be an easy
thing to do, if the function interactive wasn't a primitive function
...
`with_mock` cannot be used for this function. To work around this we must
mock the function by hand;
...
* The function `interactive` has been assigned to variable
  `interactive_original`
* `unlockBinding` has allowed us to change the function `interactive`, and
  `assign` makes sure it will always be `TRUE`
* `on.exit` makes sure that after the test has run, this function is
  changed back, and within this `lockBinding` stops any changes being made
  to the function

Now, we can add this into our test for `BROWSE`; where we only need to test
that the URL called as an argument of `BROWSE`, is passed down to
`utils::browseURL`.
...
We have now successfully tested `BROWSE` by mocking both `utils::browseURL`
and `interactive`, so that this test will pass without any internet usage
and from any OS.
